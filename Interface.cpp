	...
	last_item_id(0)
{
	...
	RoomID_Room_rel [0] = *new Room(0, "DEPO",RoomID_ItemID_rel[0]);
	last_room_id++;
}
...
int Interface::addRoom(string name)
{
	...
	RoomID_Room_rel [last_room_id] = *new Room(last_room_id, name, RoomID_ItemID_rel [last_room_id]);
	last_room_id++;
...
{
	if ( ItemID_Item_rel[itemId].ID == -1 )
	...
	unsigned int i=0;
	while (RoomID_ItemID_rel[0] [i] != itemId ||
...
bool Interface::itemReturn(int itemId)
{
	...
	unsigned int i=0;
	while (RoomID_ItemID_rel[roomId][i] != itemId ||
...
bool Interface::loadState()
{
    ...
    while(getline(load, buffer))
    {
        ...
        if ( room_id != 0 ) //Jeżeli id=0 nie robić nowego depo
            RoomID_Room_rel [room_id] = *new Room(room_id,name, RoomID_ItemID_rel[room_id]);
    ...
    while(getline(load, buffer))
    {
        ...
        RoomID_ItemID_rel [room_id] = RoomID_Room_rel [room_id].list;
